...
internal static class FlightLabSource
{
    ...
    internal static LiveReading ParseFrame(FlightLabFrame frame)
    {
        ...
        return new LiveReading(frame.Aircraft.Trim(),new Telemetry(frame.SimulatorUtc,frame.OnGround,frame.GroundSpeedKnots,
            frame.ParkingBrake,frame.EnginesRunning,frame.Paused,frame.Slewing),
            new FlightEvidence(SimulationRunning: true, PlanStatus: "Flight Lab does not provide a geographic route."),
            Source: "FLIGHT LAB", ScenarioEvent: frame.ScenarioEvent);
    }
}
