...
internal static class DashboardSmokeTest
{
    internal static async Task RunAsync(MainWindow window, string outputDirectory)
    {
        ...
        window.UpdateHeroProgress(58,false);window.UpdateLayout();Check(window.HeroProgressAircraftTranslate.X>window.HeroProgressTrack.ActualWidth*.5,"Dashboard hero uses the matching aircraft marker at continuous flight progress");
        ...
        systemsEvents.AddRange(systemsMonitor.Observe(new(eventTime.AddSeconds(1),true,0,true,true,FuelTotalWeightPounds:33990,RunningEngineCount:1,RunningEngineMask:1,FlapsExtendedRatio:.25),FlightPhase.AtGate,null,0,trackingPlan,null));
        systemsEvents.AddRange(systemsMonitor.Observe(new(eventTime.AddSeconds(2),true,0,true,true,FuelTotalWeightPounds:33980,RunningEngineCount:2,RunningEngineMask:3,FlapsExtendedRatio:.25),FlightPhase.AtGate,null,0,trackingPlan,null));
        ...
        Check(systemsEvents.Any(entry=>entry.Title=="Engine 1 on")&&systemsEvents.Any(entry=>entry.Title=="Engine 2 on")&&systemsEvents.Any(entry=>entry.Title=="Flaps changed")&&systemsEvents.Any(entry=>entry.Title=="Touchdown"&&entry.Summary.Contains("fuel burn")),"Event Log captures individual engines, stabilized flap settings, touchdown rate, schedule variance, and SimBrief fuel variance");
