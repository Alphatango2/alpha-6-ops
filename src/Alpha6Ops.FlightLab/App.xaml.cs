    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        if(e.Args.Length==2&&e.Args[0]=="--smoke-test")
        {
            try
            {
                ...
                if(!window.TelemetryText.Text.Contains("Route",StringComparison.Ordinal))throw new InvalidOperationException("Flight Lab did not publish route progress for the live tracker.");
