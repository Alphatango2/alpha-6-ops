...
$installer = Join-Path $repo 'outputs/Alpha6OPS-Setup-0.10.exe'
$archive = Join-Path $repo 'outputs/Alpha6OPS-Desktop-0.10-win-x64.zip'
...
try {
    ...
    Compress-Archive -Path "$publish/*" -DestinationPath outputs/Alpha6OPS-Desktop-0.10-win-x64.zip -Force
    ...
    Get-FileHash outputs/Alpha6OPS-Setup-0.10.exe,outputs/Alpha6OPS-Desktop-0.10-win-x64.zip | Format-Table -AutoSize
