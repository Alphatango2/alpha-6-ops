...
Check(AircraftGroundProfiles.ForFamily(null) == AircraftGroundProfile.Default, "an unknown aircraft resolves to the default ground profile");
Check(AircraftGroundProfiles.ForFamily("Airbus A321") == AircraftGroundProfile.Default, "every family resolves to the default profile until real per-type data exists");
var enginesOptional = new AircraftGroundProfile(AircraftGroundProfile.Default.MinimumTurnMinutes, 0.5, true, false);
var turboprop = new PhaseDetector(enginesOptional);
turboprop.Observe(S(200)); turboprop.Observe(S(203));
turboprop.Observe(S(210, false)); turboprop.Observe(S(213, false));
turboprop.Observe(S(220)); turboprop.Observe(S(223));
turboprop.Observe(S(230, speed: 0, brake: true, engines: true));
Check(turboprop.Observe(S(233, speed: 0, brake: true, engines: true))?.Phase == FlightPhase.Complete, "a ground profile can allow block-in with engines still running");
var strict = new PhaseDetector();
strict.Observe(S(200)); strict.Observe(S(203));
strict.Observe(S(210, false)); strict.Observe(S(213, false));
strict.Observe(S(220)); strict.Observe(S(223));
strict.Observe(S(230, speed: 0, brake: true, engines: true));
Check(strict.Observe(S(233, speed: 0, brake: true, engines: true)) is null && strict.Phase == FlightPhase.TaxiIn, "the default profile still requires engines off for block-in");
