...
Checkpoint 5 will cover restart recovery and real-flight validation, including cleanup of Windows single-instance activation. Checkpoint 5.2 refines the tracking interface: imported assignments prefer SimBrief's descriptive aircraft variant over its short ICAO equipment code, the redundant Live Tracking badge is removed, View Flight Deck uses the themed yellow hover treatment, and both progress tracks use the same yellow and charcoal palette. The full Flight Tracking progress bar retains its filled aircraft marker and smooth continuous motion; the dashboard hero remains a clean, smoothly animated line without a marker, and its airport connector omits the redundant trailing chevron. If Alpha 6 OPS is minimized or hidden in the system tray and the user selects its pinned taskbar or Start-menu shortcut, the new launch request must signal the existing process to restore its main window, return it to a normal visible state, activate it, and bring it to the foreground. It must not open a duplicate process or leave the existing window hidden. This behavior will be tested alongside tray-icon restoration, SimConnect interruption recovery, application restart recovery, and the final simulator test flight.
