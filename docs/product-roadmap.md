...
Checkpoint 5 is implemented locally. An atomic active-flight checkpoint preserves the last simulator sample, aircraft identity, route fraction, phase, actual departure and arrival, phase milestones, detailed event list, and event-monitor state. A restart reconstructs the live rotation and tracking workspace; a reconnect retains the session and accepts the first returning simulator timestamp as a new continuity baseline while still rejecting an aircraft change. Repeated identical connection-loss events are coalesced. **Clear Active Flight** removes the assignment and in-progress checkpoint, returns both tracking views to **No Active Flight**, and leaves completed journals and flight history intact. Windows single-instance activation now uses an interprocess signal that restores the existing window even when it is hidden in the tray, with an automated two-process acceptance check. Checkpoint 5.2 also refines the interface: imported assignments prefer SimBrief's descriptive aircraft variant over its short ICAO equipment code, the redundant Live Tracking badge is removed, View Flight Deck uses the themed yellow hover treatment, and both progress tracks use the same yellow and charcoal palette. The full tracker retains its filled aircraft marker; the dashboard hero remains a clean, smoothly animated line. Automated replay, Flight Lab boundary, persistence/restart, tray restoration, and second-launch activation checks pass. Final native SimConnect acceptance is performed with the real-flight checklist before publishing.

Checkpoint 6.1 is implemented locally: the dashboard **Fly** tile is replaced by **Pilot Logbook** with a dedicated cockpit-logbook image. It opens a native workspace inside the main shell and presents the existing local flight-history records with callsign, departure, arrival, aircraft, UTC date, source, and recording state. Summary counts distinguish completed from in-progress or interrupted sessions, and a defined empty state appears before the first record. Richer live-flight storage, filters, record details, and dispatch-controlled PIREP status remain in later Checkpoint 6 work.
