...
Checkpoint 4 is implemented locally: flight progress now drives distance remaining, an airborne ETA, and signed early/late schedule variance. SimConnect and Flight Lab v3 supply indicated airspeed, vertical speed, gear position, and altitude above ground in addition to the Checkpoint 3 telemetry. A stable event monitor records connection and monitoring state; engine start, pushback, taxi and takeoff roll; liftoff, climb and 10,000-foot crossing; top of climb/cruise and top of descent; approach, gear down and final; touchdown, go-around, runway vacated, taxi-in, shutdown, parking brake and block-in; route deviation/rejoin; destination changes and diversions; pause/slew, clock discontinuity, aircraft changes, and completion. Noisy state events require consecutive samples. The visible panel keeps the latest 12 milestones and expands each to show relevant simulator UTC, position, altitude, speeds, vertical speed, heading, route progress, distance, ETA variance, and nearby SimBrief fix. The JSONL journal retains every telemetry sample and every enriched tracking event.
