...
Checkpoint 3 is implemented locally: live SimConnect latitude, longitude, altitude, and true heading now enter the shared telemetry stream and flight journal. The globe moves and rotates a clean filled yellow aircraft marker, colors the completed route cyan while retaining the remaining SimBrief route in yellow, and overlays the actual flown track. Flight Lab protocol v2 supplies normalized route progress through that same live-reading boundary, allowing its automatic and manual phases to move the aircraft over whichever SimBrief route is active. The lower operational progress bar now uses the same filled aircraft treatment and advances with flight progress.

Checkpoint 4 will expand flight phases, progress, ETA, and the chronological Flight Event Log. The visible tracker will keep a concise operational record of major milestones: monitor armed; SimConnect connection, loss, and recovery; pushback; block-out and taxi; takeoff roll and liftoff; initial climb; 10,000-foot crossings; top of climb and cruise established; top of descent; approach, gear extension, and final approach; touchdown with landing speed; go-around; runway vacated; taxi-in, block-in, engine shutdown, and parking brake; diversion or destination change; route deviation and rejoin; pause or slew; simulator-clock discontinuity; aircraft change; and flight completion. Events will carry the relevant simulator UTC time, position, altitude, speed, vertical speed, heading, distance remaining, route progress, schedule variance, airport, and nearby SimBrief waypoint where available. The panel will prioritize roughly 10–15 major events with expandable details, while the JSONL diagnostic journal retains full telemetry samples and technical events for troubleshooting and Flight Lab stress testing.

Checkpoint 5 will cover restart recovery and real-flight validation, including cleanup of Windows single-instance activation. If Alpha 6 OPS is minimized or hidden in the system tray and the user selects its pinned taskbar or Start-menu shortcut, the new launch request must signal the existing process to restore its main window, return it to a normal visible state, activate it, and bring it to the foreground. It must not open a duplicate process or leave the existing window hidden. This behavior will be tested alongside tray-icon restoration, SimConnect interruption recovery, application restart recovery, and the final simulator test flight.
