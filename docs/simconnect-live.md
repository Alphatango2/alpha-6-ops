...
Live monitoring arms only when stationary on ground with parking brake set, unpaused and not slewing. The Core phase detector produces live milestones. Aircraft-title changes or clock reversal invalidate the session and require reconnecting at the gate. The demo rotation remains untouched: airport/aircraft assignment matching and live schedule anchoring are not implemented. Tracking all engines beyond four, aircraft-specific parking procedures, clock jumps forward and reconnect recovery need additional work before operational use.
