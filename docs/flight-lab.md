...
Flight Lab v3 publishes normalized route progress plus altitude, indicated airspeed, vertical speed, landing-gear and flap position, altitude above ground, pitch, bank, fuel weight, and individual engine state. Its manual phases and automatic flight move the live aircraft marker along the SimBrief route and exercise the expanded operational-event detector through the same boundary used by native SimConnect. Fault controls freeze telemetry, drop the pipe connection, jump simulator UTC by ten minutes, change aircraft identity, take the virtual simulator link offline, and restart it. Diversion is recorded as a Flight Lab scenario event; route reassignment remains future work.
