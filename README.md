...
The local v0.13.0 work now includes **Flight Tracking Checkpoints 1 and 2**: Flights is renamed in the navigation, the full tracker opens inside the main application, dashboard actions link to it, View Flight Deck replaces Start Preflight, and assignments have a dedicated tracking layout plus a defined No Active Flight state. Fresh SimBrief imports retain airport and navlog coordinates and plot the complete planned route on a themed, pan-and-zoom globe with automatic framing and date-line handling. Live SimConnect aircraft movement follows in Checkpoint 3.
