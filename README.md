...
The local v0.13.0 work has begun with the **Flight Tracking foundation**: Flights is renamed in the navigation, the full tracker now opens inside the main application, dashboard flight details and the compact network map link to it, View Flight Deck replaces Start Preflight, and assignments have a dedicated tracking layout plus a defined No Active Flight state. Route plotting and live positional telemetry follow in later checkpoints.
